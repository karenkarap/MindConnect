// components/SvgSprite.tsx
const SvgSprite = () => {
  return (
    <svg style={{ display: 'none' }} xmlns="http://www.w3.org/2000/svg">
      <defs>
        <symbol id="arrow" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M12.6216 0.860936C12.5447 0.314027 12.0391 -0.0670213 11.4921 0.00984164L2.57973 1.2624C2.03282 1.33926 1.65177 1.84493 1.72864 2.39184C1.8055 2.93875 2.31117 3.3198 2.85808 3.24294L10.7802 2.12955L11.8936 10.0517C11.9705 10.5986 12.4761 10.9797 13.023 10.9028C13.57 10.8259 13.951 10.3203 13.8741 9.77335L12.6216 0.860936ZM1.59728 15.9774L12.43 1.60193L10.8327 0.398295L9.59635e-06 14.7737L1.59728 15.9774Z"
            fill="#FBFBFB"
          />
        </symbol>
        <symbol id="question" viewBox="0 0 10 17" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M2.5 5.3125C2.5 4.14043 3.39687 3.1875 4.5 3.1875H5.5C6.60313 3.1875 7.5 4.14043 7.5 5.3125V5.43203C7.5 6.15586 7.15313 6.82988 6.58125 7.21836L5.2625 8.11817C4.8753 8.38268 4.5568 8.74625 4.33622 9.17554C4.11564 9.60483 4.00003 10.0861 4 10.5752V10.625C4 11.2127 4.44688 11.6875 5 11.6875C5.55312 11.6875 6 11.2127 6 10.625V10.5785C6 10.3063 6.13125 10.0539 6.34375 9.90781L7.6625 9.00801C8.80625 8.22442 9.5 6.87969 9.5 5.43203V5.3125C9.5 2.96504 7.70937 1.0625 5.5 1.0625H4.5C2.29063 1.0625 0.5 2.96504 0.5 5.3125C0.5 5.9002 0.946875 6.375 1.5 6.375C2.05313 6.375 2.5 5.9002 2.5 5.3125ZM5 15.9375C5.33152 15.9375 5.64946 15.7976 5.88388 15.5485C6.1183 15.2994 6.25 14.9616 6.25 14.6094C6.25 14.2571 6.1183 13.9193 5.88388 13.6703C5.64946 13.4212 5.33152 13.2813 5 13.2813C4.66848 13.2813 4.35054 13.4212 4.11612 13.6703C3.8817 13.9193 3.75 14.2571 3.75 14.6094C3.75 14.9616 3.8817 15.2994 4.11612 15.5485C4.35054 15.7976 4.66848 15.9375 5 15.9375Z"
            fill="#FBFBFB"
          />
        </symbol>

        <symbol id="person" viewBox="0 0 17 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M11.6043 11.9741L11.1765 13.5708L2.16847e-05 10.576L0.427841 8.97939C0.427841 8.97939 1.28348 5.7861 6.87173 7.28347C12.46 8.78083 11.6043 11.9741 11.6043 11.9741ZM10.8424 3.64138C10.9904 3.08875 10.9713 2.50463 10.7874 1.96287C10.6035 1.42111 10.2631 0.946053 9.8092 0.597768C9.3553 0.249483 8.80832 0.0436141 8.23743 0.00619567C7.66653 -0.0312229 7.09736 0.101489 6.60189 0.38755C6.10642 0.67361 5.7069 1.10017 5.45386 1.61329C5.20082 2.12641 5.10562 2.70304 5.18029 3.27027C5.25497 3.83749 5.49617 4.36984 5.8734 4.79998C6.25062 5.23012 6.74693 5.53875 7.29955 5.68682C8.0406 5.88539 8.83018 5.78144 9.49458 5.39784C10.159 5.01425 10.6438 4.38243 10.8424 3.64138ZM12.4121 8.768C12.8011 9.2793 13.0733 9.86964 13.2096 10.4975C13.3459 11.1253 13.343 11.7754 13.201 12.4019L12.7732 13.9986L15.9665 14.8542L16.3943 13.2576C16.3943 13.2576 17.1708 10.3597 12.4121 8.768ZM13.5869 1.38203C13.0382 1.23173 12.4568 1.25202 11.9199 1.44019C12.2233 2.24769 12.2663 3.12989 12.0431 3.96311C11.8198 4.79633 11.3414 5.5388 10.6749 6.08642C11.0458 6.51782 11.5392 6.8261 12.0895 6.97028C12.8305 7.16884 13.6201 7.06489 14.2845 6.6813C14.9489 6.2977 15.4337 5.66589 15.6323 4.92484C15.8309 4.18379 15.7269 3.39421 15.3433 2.72981C14.9597 2.0654 14.3279 1.58059 13.5869 1.38203Z"
            fill="#FBFBFB"
          />
        </symbol>

        <symbol id="check" viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M2.5 5L0 7.5L7.5 15L20 2.5L17.5 0L7.5 10L2.5 5Z"
            fill="#54BE96"
          />
        </symbol>

        <symbol id="close" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M24 8L8 24"
            stroke="#191A15"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M8 8L24 24"
            stroke="#191A15"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </symbol>

        <symbol id="eyeClose" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clipPath="url(#clip0_33_590)">
            <path
              d="M14.95 14.95C13.5255 16.0358 11.791 16.6374 10 16.6667C4.16671 16.6667 0.833374 10 0.833374 10C1.86995 8.06825 3.30765 6.38051 5.05004 5.05M8.25004 3.53333C8.82365 3.39907 9.41093 3.33195 10 3.33333C15.8334 3.33333 19.1667 10 19.1667 10C18.6609 10.9463 18.0576 11.8373 17.3667 12.6583M11.7667 11.7667C11.5378 12.0123 11.2618 12.2093 10.9552 12.3459C10.6485 12.4826 10.3175 12.556 9.98178 12.562C9.64611 12.5679 9.31268 12.5061 9.00138 12.3804C8.69009 12.2547 8.40731 12.0675 8.16991 11.8301C7.93252 11.5927 7.74537 11.31 7.61963 10.9987C7.4939 10.6874 7.43215 10.3539 7.43807 10.0183C7.44399 9.68258 7.51747 9.35154 7.65411 9.04487C7.79075 8.73821 7.98775 8.46221 8.23338 8.23333"
              stroke="#191A15"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M0.833374 0.833336L19.1667 19.1667"
              stroke="#191A15"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </g>
          <defs>
            <clipPath id="clip0_33_590">
              <rect width="20" height="20" fill="white" />
            </clipPath>
          </defs>
        </symbol>

        <symbol id="eyeOpen" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clipPath="url(#clip0_33_593)">
            <path
              d="M0.833374 10C0.833374 10 4.16671 3.33334 10 3.33334C15.8334 3.33334 19.1667 10 19.1667 10C19.1667 10 15.8334 16.6667 10 16.6667C4.16671 16.6667 0.833374 10 0.833374 10Z"
              stroke="#191A15"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M10 12.5C11.3807 12.5 12.5 11.3807 12.5 10C12.5 8.61929 11.3807 7.5 10 7.5C8.61929 7.5 7.5 8.61929 7.5 10C7.5 11.3807 8.61929 12.5 10 12.5Z"
              stroke="#191A15"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </g>
          <defs>
            <clipPath id="clip0_33_593">
              <rect width="20" height="20" fill="white" />
            </clipPath>
          </defs>
        </symbol>

        <symbol id="star" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M8.88965 4.12578C9.0488 4.42729 9.33887 4.63791 9.6748 4.69609L13.4746 5.35234L10.7871 8.11992C10.5496 8.36449 10.4388 8.7053 10.4873 9.04277L11.0361 12.8602L7.5752 11.159L7.45801 11.1092C7.21993 11.0246 6.95976 11.0246 6.72168 11.1092L6.60449 11.159L3.14258 12.8602L3.69238 9.04277C3.74091 8.7053 3.6301 8.36449 3.39258 8.11992L0.704102 5.35234L4.50488 4.69609C4.84082 4.63791 5.13088 4.42729 5.29004 4.12578L7.08984 0.714645L8.88965 4.12578Z"
            fill="#FFC531"
            stroke="#FFC531"
            strokeWidth="1.2"
          />
        </symbol>

        <symbol id="like" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M22.5766 4.99417C22.0233 4.44059 21.3663 4.00145 20.6433 3.70184C19.9202 3.40223 19.1452 3.24802 18.3625 3.24802C17.5798 3.24802 16.8047 3.40223 16.0817 3.70184C15.3586 4.00145 14.7016 4.44059 14.1483 4.99417L13 6.1425L11.8516 4.99417C10.734 3.8765 9.21809 3.2486 7.63747 3.2486C6.05685 3.2486 4.54097 3.8765 3.4233 4.99417C2.30563 6.11183 1.67773 7.62771 1.67773 9.20833C1.67773 10.789 2.30563 12.3048 3.4233 13.4225L4.57163 14.5708L13 22.9992L21.4283 14.5708L22.5766 13.4225C23.1302 12.8692 23.5693 12.2122 23.869 11.4891C24.1686 10.7661 24.3228 9.99103 24.3228 9.20833C24.3228 8.42564 24.1686 7.65061 23.869 6.92753C23.5693 6.20445 23.1302 5.54749 22.5766 4.99417Z"
            stroke="#191A15"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </symbol>

        <symbol
          id="dropdownArrow"
          viewBox="0 0 20 20"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M5 7.5L10 12.5L15 7.5"
            stroke="#FBFBFB"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </symbol>

        <symbol id="profile" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M12 4C13.0609 4 14.0783 4.42143 14.8284 5.17157C15.5786 5.92172 16 6.93913 16 8C16 9.06087 15.5786 10.0783 14.8284 10.8284C14.0783 11.5786 13.0609 12 12 12C10.9391 12 9.92172 11.5786 9.17157 10.8284C8.42143 10.0783 8 9.06087 8 8C8 6.93913 8.42143 5.92172 9.17157 5.17157C9.92172 4.42143 10.9391 4 12 4ZM12 14C16.42 14 20 15.79 20 18V20H4V18C4 15.79 7.58 14 12 14Z"
            fill="#FBFBFB"
          />
        </symbol>

        <symbol id="activeLike" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            d="M22.5766 4.99417C22.0233 4.44059 21.3663 4.00145 20.6433 3.70184C19.9202 3.40223 19.1452 3.24802 18.3625 3.24802C17.5798 3.24802 16.8047 3.40223 16.0817 3.70184C15.3586 4.00145 14.7016 4.44059 14.1483 4.99417L13 6.1425L11.8516 4.99417C10.734 3.8765 9.21809 3.2486 7.63747 3.2486C6.05685 3.2486 4.54097 3.8765 3.4233 4.99417C2.30563 6.11183 1.67773 7.62772 1.67773 9.20833C1.67773 10.789 2.30563 12.3048 3.4233 13.4225L4.57163 14.5708L13 22.9992L21.4283 14.5708L22.5766 13.4225C23.1302 12.8692 23.5693 12.2122 23.869 11.4891C24.1686 10.7661 24.3228 9.99103 24.3228 9.20833C24.3228 8.42564 24.1686 7.65061 23.869 6.92753C23.5693 6.20445 23.1302 5.54749 22.5766 4.99417Z"
            fill="#54BE96"
            stroke="#54BE96"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </symbol>
      </defs>
    </svg>
  );
};

export default SvgSprite;
